...
function baseParams(rng: RNG, family: Family, t: number): ShapeParams {
  ...
  return {
    ...
    rotation: family === "circle" || family === "ring" ? 0 : rng.range(-12, 12),
    scale: 1,
    offsetX: 0,
    offsetY: 0,
    mirror: false,
...
function applyDiff(
  ...
  switch (diff) {
    ...
    case "saturation":
      ...
      break;
    case "scale":
      odd.scale = lerp(1.24, 1.055, t);
      break;
    case "rotation":
      odd.rotation += sign * lerp(20, 3.4, t);
      break;
    case "stroke":
      if (base.strokeOn) {
...
export function generateRound(
  ...
  if (roundIndex < 2 && rng.chance(0.55)) {
    family = rng.pick(["card", "chevron", "bars", "slash", "triangle", "star"]);
  }
